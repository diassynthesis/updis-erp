...
class InternalHome(openerp.addons.web.http.Controller):
    ...
    def _upload_file_ftp(self, file_data, file_name):
        ...
        output = open(HOME + '/' + file_name, 'wb')
        output.write(file_data)
        output.close()
        try:
            host = ftputil.FTPHost('10.100.100.196', 'ftpuser', 'updis_ftp_2013')
            host.upload(HOME + '/' + file_name,'/'+file_name)
        except Exception, e:
    ...
    def upload_image(self, req, qqfile):
        try:
            ...
            # model, id, field, **kw
            url = '%sweb/binary/image?session_id=%s&model=ir.attachment&field=datas&id=%s' % (
                req.httprequest.url_root, req.session_id, args['id'])
            args['url'] = url
        except Exception, e:
            args = {'error': e.message}
    ...
    def upload_file(self, req, qqfile):
        try:
            ...
            url = '%sweb/binary/saveas?session_id=%s&model=ir.attachment&field=datas&filename_field=datas_fname&id=%s' % (
                req.httprequest.url_root, req.session_id, args['id'])
            args['url'] = url
        except Exception, e:
            args = {'error': e.message}
